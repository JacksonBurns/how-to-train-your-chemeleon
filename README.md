...
If you want to _exactly_ re-run the original code you should follow that link, otherwise the code that is in this repository is much better suited for making new `CheMeleon`-inspired models.
...
With `python==3.13` one just needs to `pip install 'chemprop>=2.2.3' zarr` and the corresponding feature calculator:
...
Simply set your preferred hyperparameters in `config.py` and then execute `train.py`.
